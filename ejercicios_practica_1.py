...
def ej2():
    ...
    # Comenzar aquí, recuerde el identado dentro de esta funcion
    with open('stock.csv') as fi:
         while True:
            producto = str(input("Ingrese el producto que desea agregar al stock o ingrese FIN para finalizar: "))
            if producto.upper() == 'FIN':
                 break
            elif producto not in stock:
                print("El producto no se encuentra en en el stock")        
            else:
                 cantidad = int(input("Ingrese la cantidad de producto que desea agregar: "))
